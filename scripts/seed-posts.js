#!/usr/bin/env node
/**
 * ORRA Fast Seed — Creates 100 posts + comments instantly (no image generation).
 * Then run scripts/generate-images.js separately to add AI photos.
 */
...
const BOT_USERS = [
  { id: 'u1',  name: 'Jessica Art' },
  { id: 'u2',  name: 'David Chen' },
  { id: 'u3',  name: 'Sarah Kim' },
  { id: 'u4',  name: 'Marcus Rivera' },
  { id: 'u5',  name: 'Elena Rodriguez' },
  { id: 'u6',  name: 'Tech Daily' },
  { id: 'u7',  name: 'Wellness Guru' },
  { id: 'u8',  name: 'Cyber Drifter' },
  { id: 'u9',  name: 'Music Central' },
  { id: 'u10', name: 'Luna Sky' },
  { id: 'u11', name: 'Kai Storm' },
  { id: 'u12', name: 'Nova Blaze' },
  { id: 'u13', name: 'Zara Miles' },
  { id: 'u14', name: 'Jay Parker' },
  { id: 'u15', name: 'Maya Chen' },
  { id: 'u16', name: 'Dre Williams' },
];

const REACTION_TYPES = ['heart', 'wow', 'omg', 'wtf', 'laugh', 'sad'];

// 100 diverse posts — each with a unique image prompt for later generation
const POST_TEMPLATES = [
  ...
  // FASHION & STYLE (10)
  { text: "New fit check! This look took 3 stores and 47 changing rooms to find", vibeTag: 'dramatic', prompt: 'A stylish streetwear outfit flat lay, trendy sneakers, oversized hoodie, accessories, fashion photography, clean background' },
...
async function main() {
  console.log('=== ORRA Fast Seed — Creating 100 posts + comments ===\n');
  ...
  try {
    ...
    console.log('  ✓ Cleared all posts, comments, likes, reposts, saves\n');
  } catch (err) {
    console.error('  ✗ Clean failed:', err.message);
  }

  // Step 2: Create 100 posts (text-only, images will be added by generate-images.js)
  console.log('[2/4] Creating 100 diverse posts...');
  let templates = shuffleArray(POST_TEMPLATES);
  // Expand to 100 by cycling
  while (templates.length < 100) {
    templates = [...templates, ...shuffleArray(POST_TEMPLATES)];
  }
  ...
  for (let i = 0; i < templates.length; i++) {
    ...
    const hasPrompt = !!template.prompt;
    ...
    try {
      const post = await db.post.create({
        data: {
          ...
          type: 'text', // Will be updated to 'image' when images are added
          authorId: author.id,
          likesCount: randomInt(5, 80),
          commentsCount: 0,
          sharesCount: randomInt(2, 25),
          createdAt,
        },
      });
  ...
  // Step 3: Add 3-7 comments per post
  console.log('[3/4] Adding comments...');
  let totalComments = 0;
  for (const { post } of createdPosts) {
    const numComments = randomInt(3, 7);
    ...
    for (let j = 0; j < numComments; j++) {
      try {
        ...
        await db.comment.create({
          data: {
            text: randomItem(COMMENT_POOL),
            postId: post.id,
            authorId: commentAuthors[j % commentAuthors.length].id,
            likesCount: randomInt(0, 12),
  ...
  // Step 4: Add reactions to posts and comments
  console.log('[4/4] Adding reactions...');
    ...
    for (const reactor of reactors) {
      try {
        await db.like.create({
          data: { userId: reactor.id, targetId: post.id, targetType: 'post', reaction: randomItem(REACTION_TYPES) },
        });
        totalReactions++;
      } catch (err) {}
    }
  ...
  // Comment reactions
  const allComments = await db.comment.findMany({ take: 300 });
  for (const comment of allComments) {
    if (Math.random() < 0.35) {
      try {
        await db.like.create({
          data: { userId: randomItem(BOT_USERS).id, targetId: comment.id, targetType: 'comment', reaction: randomItem(REACTION_TYPES) },
        });
        totalReactions++;
      } catch (err) {}
    }
  ...
  // Save post data for image generation
  const imageData = createdPosts
    .filter(({ template }) => template.prompt)
    .map(({ post, template }) => ({ postId: post.id, prompt: template.prompt }));

  const imageQueuePath = path.join(PROJECT_ROOT, '.image-queue.json');
  fs.writeFileSync(imageQueuePath, JSON.stringify(imageData, null, 2));
  console.log(`Image queue saved: ${imageData.length} images to generate`);
  console.log(`Run: node scripts/generate-images.js\n`);
  ...
  console.log(`Comments: ${stats[1]}`);
  console.log(`Reactions: ${stats[2]}`);
  console.log(`Image queue: ${imageData.length} prompts`);
