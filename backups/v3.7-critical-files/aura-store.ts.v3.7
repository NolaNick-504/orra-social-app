  ...
  persist(
    (set, get) => ({
      ...
      toggleLike: (postId) =>
        set((s) => {
          ...
          const state: Partial<AuraState> = { likedPosts: newLiked, likedPostsEarned: newEarned };
          ...
          if (!wasLiked && !newEarned.has(postId)) {
            ...
            state.auraXP = s.auraXP + 2;
            if (s.auraXP + 2 >= 1000) {
              state.auraLevel = s.auraLevel + 1;
              state.auraXP = (s.auraXP + 2) - 1000;
              state.customNotifications = [{
                id: `lvl-${Date.now()}`,
                userId: 'system',
                userName: 'ORRA',
                userAvatar: '/api/uploads?path=images/orra-logo.png',
                action: `Level Up! You're now Level ${s.auraLevel + 1}!`,
                time: 'Just now',
                type: 'levelup',
                read: false,
      ...
      toggleFollow: (userId) =>
        set((s) => {
          ...
          if (!wasFollowed && !newEarned.has(userId)) {
            ...
            state.auraXP = s.auraXP + 5;
            if (s.auraXP + 5 >= 1000) {
              state.auraLevel = s.auraLevel + 1;
              state.auraXP = (s.auraXP + 5) - 1000;
            }
          }
          return state;
        }),
      ...
      addComment: (postId, userId, userName, userAvatar, text) =>
        set((s) => {
          ...
          return {
            ...
            auraLevel: levelUp ? s.auraLevel + 1 : s.auraLevel,
            customNotifications: levelUp ? [{
              id: `lvl-${Date.now()}`,
              userId: 'system',
              userName: 'ORRA',
              userAvatar: '/api/uploads?path=images/orra-logo.png',
      ...
      earnTokens: (amount, reason) =>
        set((s) => {
          const newXP = s.auraXP + Math.floor(amount / 2);
          const levelUp = newXP >= 1000;
          return {
      ...
      addXP: (amount) =>
        set((s) => {
          const newXP = s.auraXP + amount;
          const levelUp = newXP >= 1000;
          if (levelUp) {
            return {
              auraXP: newXP - 1000,
              auraLevel: s.auraLevel + 1,
              customNotifications: [{
                id: `lvl-${Date.now()}`,
                userId: 'system',
                userName: 'ORRA',
      ...
      joinHub: (hubId) =>
        set((s) => {
          ...
          return {
            ...
            auraXP: s.auraXP + 10 >= 1000 ? (s.auraXP + 10) - 1000 : s.auraXP + 10,
            auraLevel: s.auraXP + 10 >= 1000 ? s.auraLevel + 1 : s.auraLevel,
            customNotifications: [{
              id: `hub-${Date.now()}`,
              userId: 'system',
              userName: 'ORRA',
      ...
      voteEntry: (entryId) =>
        set((s) => {
          ...
          if (!newEarned.has(entryId)) {
            ...
            state.auraTokens = s.auraTokens + 1;
            state.auraXP = s.auraXP + 2 >= 1000 ? (s.auraXP + 2) - 1000 : s.auraXP + 2;
            state.auraLevel = s.auraXP + 2 >= 1000 ? s.auraLevel + 1 : s.auraLevel;
          }
          return state;
        }),
      ...
      markReelViewed: (reelId) =>
        set((s) => {
          ...
          return {
            ...
            auraTokens: s.auraTokens + 1,
            auraXP: s.auraXP + 1 >= 1000 ? (s.auraXP + 1) - 1000 : s.auraXP + 1,
            auraLevel: s.auraXP + 1 >= 1000 ? s.auraLevel + 1 : s.auraLevel,
          };
        }),
    ...
    }),
    {
      ...
      partialize: (state) => {
        ...
        return {
          version: 7, // Storage version — bump when data model changes to force clean state
          ...
          // Limit comments to last 100 threads, each with last 10 comments to prevent bloat
          comments: Object.fromEntries(
            Object.entries(state.comments)
              .sort(([, a], [, b]) => (b[b.length - 1]?.createdAt ?? 0) - (a[a.length - 1]?.createdAt ?? 0))
              .slice(0, 100)
              .map(([key, comments]) => [key, comments.slice(-10)])
          ),
