...
// Mobile bottom nav with scroll-aware hide/show behavior
function MobileBottomNav({ currentView, setView, toggleCreatePost, setMobileMenuOpen }: {
  currentView: NavView;
  ...
  const isHidden = scrollDir === 'down' && !isAtTop;

  return (
    <nav
      className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 glass-panel border-t border-white/10 safe-area-bottom transition-transform duration-300 ease-in-out ${
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Nav - 5 items + Menu — hides on scroll down, shows on scroll up */}
      <MobileBottomNav currentView={currentView} setView={setView} toggleCreatePost={toggleCreatePost} setMobileMenuOpen={setMobileMenuOpen} />
