...
import { usePosts, usePost, useCreatePost, useToggleLike, useToggleSave, useToggleRepost, useVotePoll, useCreateComment, useComments } from '@/lib/api-hooks';
...
const REACTION_TYPES = [
  ...
  { key: 'laughing', emoji: '😂', label: 'LOL', color: 'text-amber-400', bgColor: 'bg-amber-500/10', activeBg: 'bg-amber-500/20' },
...
export function PulseFeed() {
  ...
  // When navigating from a notification (scrollToPostId is set), don't filter by vibe
  // so the target post is guaranteed to be visible
  const activeVibeFilter = scrollToPostId
    ? undefined // No filter when navigating to a specific post
    : (selectedVibes && selectedVibes.length > 0) ? selectedVibes[0] : currentVibe;
  // Fetch more posts when navigating from a notification (target post might not be on first page)
  const postsLimit = scrollToPostId ? 50 : 20;
  const { data: postsData, isLoading: postsLoading, error: postsError } = usePosts(activeVibeFilter || undefined, 1, postsLimit);

  // Auto-scroll to a specific post when navigating from a notification
  // Uses multiple retries with increasing delays to handle slow-loading posts
  useEffect(() => {
    if (!scrollToPostId) return;
    if (postsLoading) return; // Wait until posts finish loading
    ...
    const tryScroll = () => {
      ...
      if (el) {
        // Found the post — scroll to it and highlight
        el.scrollIntoView({ behavior: 'smooth', block: 'center' });
        el.classList.add('ring-2', 'ring-violet-500/50');
        setTimeout(() => {
          el.classList.remove('ring-2', 'ring-violet-500/50');
        }, 2000);
        // Auto-expand comments on the post
        if (!expandedComments.has(scrollToPostId)) {
          toggleComments(scrollToPostId);
        }
        setScrollToPostId(null); // Clear after scrolling
      } else {
        // Post not found yet — retry with increasing delay
        attempts++;
        if (attempts < maxAttempts) {
          const delay = 300 + attempts * 200; // 500ms, 700ms, 900ms, etc.
          timer = setTimeout(tryScroll, delay);
        } else {
          // Give up after max attempts — post might not exist in current feed
          setScrollToPostId(null);
        }
      }
    };

    // Start first attempt after a short delay to let React render
    timer = setTimeout(tryScroll, 300);

    return () => {
      if (timer) clearTimeout(timer);
    };
  }, [scrollToPostId, postsLoading, setScrollToPostId, expandedComments, toggleComments]);
  ...
  // When navigating from a notification, also fetch the specific post directly
  // This ensures the post is displayed even if it's not in the feed (e.g., older posts)
  const { data: notificationPost } = usePost(scrollToPostId);
  ...
  const apiPosts = (postsData?.posts || []).map((post) => {
    ...
    // Echo info: if this post has echoedBy, it's an echo entry
    const echoedBy = (post as any).echoedBy || null;
    const echoedAt = (post as any).echoedAt || null;
    const isEcho = (post as any)._isEcho || false;
    ...
    return {
      ...
      time: timeAgo(isEcho && echoedAt ? echoedAt : post.createdAt),
      verified: post.author.verified,
      ...
      echoedBy: echoedBy ? {
        ...
        avatar: echoedBy.avatar || '/api/uploads?path=images/orra-logo.png',
  ...
  // If navigating from a notification and the target post isn't in the feed,
  // fetch it directly and prepend it to the top
  if (scrollToPostId && notificationPost) {
    const alreadyInFeed = filteredPosts.some((p) => p.id === scrollToPostId);
    if (!alreadyInFeed) {
      const images: string[] = (() => {
        try {
          const parsed = JSON.parse(notificationPost.images);
          return Array.isArray(parsed) ? parsed : [];
        } catch {
          return [];
        }
      ...
      const notifPost = {
        ...
        user: {
          id: notificationPost.author.id,
          name: notificationPost.author.name,
          handle: notificationPost.author.handle,
          avatar: notificationPost.author.avatar || '/api/uploads?path=images/orra-logo.png',
          verified: notificationPost.author.verified,
        },
        text: notificationPost.text,
        images,
        likes: notificationPost.likesCount,
        comments: notificationPost.commentsCount,
        shares: notificationPost.sharesCount,
        time: timeAgo(notificationPost.createdAt),
        verified: notificationPost.author.verified,
        type: (notificationPost.type || (images.length > 0 ? 'image' : 'text')) as string,
        vibeTag: notificationPost.vibeTag || 'hyped',
        isLiked: notificationPost.isLiked,
        myReaction: (notificationPost as any).myReaction || null,
        isSaved: notificationPost.isSaved,
        isReposted: notificationPost.isReposted,
        isUserPost: notificationPost.author.id === currentUser.id,
        createdAt: notificationPost.createdAt,
        poll: (notificationPost as any).poll || undefined,
        isEcho: false,
        echoedBy: null,
        echoedAt: null,
      };
      filteredPosts = [notifPost, ...filteredPosts];
    }
  }

  if (selectedVibes && selectedVibes.length > 0) {
    // Don't filter out the notification target post
    filteredPosts = filteredPosts.filter((p) =>
      selectedVibes.includes(p.vibeTag) || p.id === scrollToPostId
    );
  }
  if (searchQuery.trim()) {
  ...
  const handleEcho = (postId: string) => {
    toggleRepost(postId);
    const wasReposted = repostIds.has(postId);
    // Persist to database so other users see the repost
    toggleRepostApi.mutate({ postId }, {
      onSuccess: () => {
        // Refresh the feed so the echo entry appears at the top (or is removed)
  ...
  return (
    <div className="fade-in space-y-4 pb-4">
      ...
      <button
        ...
        className="w-full rounded-2xl overflow-hidden hover:ring-2 hover:ring-fuchsia-500/30 transition-all group relative shadow-lg shadow-fuchsia-900/20"
      >
        <div className="relative h-32 md:h-40">
          {/* Digital artwork background */}
          <img
            src="/api/uploads?path=images/dance-banner.png&v=20260511a"
            alt="ORRA Dance Off"
          ...
          <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between z-10">
            <div>
              ...
              <p className="text-xl md:text-2xl font-black text-white leading-tight drop-shadow-lg">ORRA DANCE OFF</p>
              <p className="text-[11px] text-slate-300 drop-shadow">Join the challenge & win 100K ORRA + Plaque</p>
            </div>
            <div className="px-4 py-2 rounded-xl bg-gradient-to-r from-fuchsia-600 to-violet-600 text-white text-sm font-bold group-hover:opacity-90 transition-all shadow-lg shadow-fuchsia-900/40">
              Join &rarr;
            </div>
          </div>
        </div>
      ...
      <button
        ...
        className="w-full rounded-2xl overflow-hidden hover:ring-2 hover:ring-violet-500/30 transition-all group shadow-lg shadow-violet-900/20"
      >
        <div className="relative h-28 md:h-32">
          {/* Digital artwork background */}
          <img
            src="/api/uploads?path=images/arena-banner.png&v=20260511a"
            alt="Game Arena"
            className="absolute inset-0 w-full h-full object-cover"
          />
          {/* Gradient overlays for text readability */}
          ...
          <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between z-10">
            <div>
              ...
              <p className="text-lg font-black text-white leading-tight drop-shadow-lg">GAME ARENA</p>
              <p className="text-[11px] text-slate-300 drop-shadow">6 new games with friends + ORRA rewards</p>
            </div>
            <div className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white text-xs font-bold group-hover:opacity-90 transition-all shadow-lg shadow-violet-900/40">
              Play &rarr;
            </div>
          </div>
        </div>
      ...
      {filteredPosts.map((post) => {
        ...
        const isEcho = (post as any).isEcho === true;
        ...
        return (
          <div key={`${isEcho ? 'echo-' : ''}${post.id}`} id={`post-${post.id}`} className={`glass-panel rounded-2xl overflow-hidden hover:border-violet-500/20 transition-all relative  ${echoAnimation === post.id ? 'echo-ripple' : ''}`}>
            ...
            {/* Fallback Echo Indicator for non-echo-card posts (profile view) */}
            {!isEcho && isReposted && !isShared && (
            ...
            {/* Main Post Header — for echo cards this is the ECHOER's info, making it "their post" */}
            <div className="flex items-center gap-3 p-4 pb-2">
              <button
                onClick={() => { setViewingUser(headerUser.id); setView('profile'); }}
                className="flex-shrink-0 hover:opacity-80 transition-opacity"
              >
                <AuraGlowAvatar src={headerUser.avatar} alt={headerUser.name} level={auraLevel} vibeTag={currentVibe || undefined} />
              </button>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500">
                  ...
                  {post.type === 'text' && !isEcho && <span className="text-slate-600">•</span>}
                  {post.type === 'text' && !isEcho && <span className="text-slate-600">Pulse</span>}
                  ...
                  {post.type === 'image' && !isEcho && <span className="text-slate-600">•</span>}
                  {post.type === 'image' && !isEcho && <span className="text-emerald-400">Photo</span>}
                  {post.vibeTag && (
                    <>
                ...
                <div className="flex items-center gap-2.5 px-3 py-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1">
                      ...
                      {post.verified && <BadgeCheck className="w-3 h-3 text-violet-400 fill-violet-400/20" />}
                    </div>
            ...
            <div className="flex items-center justify-between px-4 py-2.5 border-t border-white/5">
              <div className="flex items-center gap-1 flex-1">
                ...
                >
                  <Waves className={`w-5 h-5 transition-all ${isReposted && !isEcho ? 'text-emerald-400' : 'text-slate-400 hover:text-emerald-400'} ${echoAnimation === post.id ? 'echo-wave-icon' : ''}`} />
                  <span className={`text-xs font-medium transition-colors ${isReposted && !isEcho ? 'text-emerald-400' : 'text-slate-400'}`}>Echo</span>
                </button>
                <button
