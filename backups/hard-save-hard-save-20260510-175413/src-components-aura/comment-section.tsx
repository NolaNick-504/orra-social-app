...
export function CommentSection({ postId, currentUser, show, onToggle, commentCount = 0 }: CommentSectionProps) {
  ...
  const handleComment = () => {
    ...
    createCommentApi.mutate({ postId, text, parentId: replyToId || undefined }, {
      onSuccess: () => {
          ...
          return (
            <div key={c.id}>
              <div className="flex gap-2 group">
                ...
                >
                  <img src={c.userAvatar || '/images/orra-logo.png'} alt={c.userName} className="w-full h-full object-cover" />
                </button>
                ...
                <div className="ml-9 mt-1 space-y-2 border-l border-white/5 pl-3">
                  {replies.map((r: any) => (
                    <div key={r.id} className="flex gap-2 group">
                      ...
                      >
                        <img src={r.userAvatar || '/images/orra-logo.png'} alt={r.userName} className="w-full h-full object-cover" />
                      </button>
                      <div className="flex-1 min-w-0">
