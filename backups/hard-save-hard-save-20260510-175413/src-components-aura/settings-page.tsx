...
import {
  ...
  Crown, Star, Waves, Moon, Monitor, RefreshCw
} from 'lucide-react';
...
export function SettingsPage() {
  ...
  const s: UserSettings = {
    ...
    feedRefreshInterval: (userSettings as any)?.feedRefreshInterval ?? 5,
      ...
      <SettingsSection title="Content & Media" icon={Play} color="green">
        ...
        <div className="p-3">
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <p className="text-sm font-medium text-white">Feed Refresh Rate</p>
          </div>
          <div className="flex gap-2">
            {[5, 15].map((mins) => {
              const active = s.feedRefreshInterval === mins;
              return (
                <button
                  key={mins}
                  onClick={() => updateUserSettings({ feedRefreshInterval: mins } as any)}
                  className={`flex-1 px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${
                    active
                      ? 'bg-green-500/20 text-green-300 border-green-500/30 shadow-lg shadow-green-500/10'
                      : 'bg-white/5 text-slate-400 border-white/10 hover:bg-white/[0.08]'
          ...
          <p className="text-[10px] text-slate-500 mt-1.5">How often the feed auto-refreshes for new posts</p>
        </div>
      </SettingsSection>
