...
export interface Notification {
  ...
  action: string;
  time: string;
  createdAt?: string; // ISO timestamp for dynamic time display
...
interface AuraState {
  ...
  currentLiveReelId: string | null;
  liveViewerCount: number;
  showLiveViewer: boolean;
  currentLiveStreamId: string | null;
  ...
  // Viewing a specific post (from notification click)
  viewingPostId: string | null;
  setViewingPostId: (postId: string | null) => void;

  // Viewing an echoed post context
  viewingEchoId: string | null;
  setViewingEchoId: (echoId: string | null) => void;
  ...
  // Comments
  addComment: (postId: string, userId: string, userName: string, userAvatar: string, text: string, parentId?: string, replyToName?: string) => void;
  ...
  clearStaleUserPosts: (apiPostIds: string[]) => void;
  ...
  // Live Streaming
  startLive: (reelId: string) => void;
  endLive: () => void;
  setLiveViewerCount: (count: number) => void;
  setShowGoLiveSetup: (show: boolean) => void;
  toggleLiveViewer: (streamId?: string | null) => void;
...
export const useAuraStore = create<AuraState>()(
  persist(
    (set, get) => ({
      ...
      // Viewing specific post (from notification)
      viewingPostId: null as string | null,

      // Viewing echo context (which repost/echo)
      viewingEchoId: null as string | null,
      ...
      toggleRepost: (postId) =>
        set((s) => {
          ...
          if (!wasReposted && !newEarned.has(postId)) {
            newEarned.add(postId);
            const { newXP, newLevel } = calculateXPLevelUp(s.auraXP, s.auraLevel, 3);
            state.auraTokens = s.auraTokens + 2;
            state.auraXP = newXP;
            state.auraLevel = newLevel;
          }
      ...
      // Comments
      addComment: (postId, userId, userName, userAvatar, text, parentId, replyToName) =>
        set((s) => {
      ...
      // Remove local posts that have been confirmed by the API (different IDs, same content)
      clearStaleUserPosts: (apiPostIds) =>
        set((s) => ({
          // Keep only local posts whose ID is NOT in the API list
          // (local posts use "up-" prefix IDs, API posts use CUIDs)
          userPosts: s.userPosts.filter((lp) => !apiPostIds.includes(lp.id)),
        })),
      ...
      // Live Streaming
      startLive: (reelId) => set({ isLiveActive: true, currentLiveReelId: reelId, liveViewerCount: 0 }),
      endLive: () => set({ isLiveActive: false, currentLiveReelId: null, liveViewerCount: 0, showGoLiveSetup: false }),
      setLiveViewerCount: (count) => set({ liveViewerCount: count }),
      setShowGoLiveSetup: (show) => set({ showGoLiveSetup: show }),
      toggleLiveViewer: (streamId) => set((s) => ({
        showLiveViewer: !s.showLiveViewer,
        currentLiveStreamId: streamId ?? null,
      })),
