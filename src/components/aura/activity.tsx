...
export function Activity() {
  ...
  const [, setTick] = useState(0); // force re-render every minute to update timestamps
  ...
  const localNotifications: NotificationItem[] = customNotifications.map((n) => ({
    ...n,
    // Use createdAt for dynamic time, fall back to static time string
  ...
  const handleNotificationClick = async (notif: NotificationItem) => {
    ...
    if (notif.postId) {
      // Use the dedicated PostDetail view — sets the post ID and navigates to it
      const store = useAuraStore.getState();
      store.setViewingPostId(notif.postId);
      store.setView('postDetail');
  ...
  const handleFollow = async (userId: string, userName: string) => {
    ...
    try {
      ...
      if (res.status === 403) {
        toggleFollow(userId);
        toast.error("Can't unfollow the ORRA Founder");
        return;
      }
          ...
          return (
            ...
            >
              ...
              <div className="relative flex-shrink-0">
                ...
                >
                  {notif.userAvatar ? (
                    <img src={resolveImageUrl(notif.userAvatar)} alt={notif.userName} className="w-full h-full object-cover" />
                  ) : (
              ...
              {notif.type === 'follow' ? (
                ...
                <div className="w-10 h-10 rounded-lg overflow-hidden ring-1 ring-white/10 flex-shrink-0 group-hover:ring-violet-500/30 transition-all">
                  <img src={resolveImageUrl(notif.thumbnail)} alt="" className="w-full h-full object-cover" />
                </div>
              ) : null}
            </div>
          );
