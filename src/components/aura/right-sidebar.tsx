                ...
                <div key={user.id} className="flex items-center gap-2">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch('/api/follows', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id }) });
                        if (res.status === 403) { toggleFollow(user.id); toast.error("Can't unfollow the ORRA Founder"); return; }
                      } catch { toggleFollow(user.id); }
