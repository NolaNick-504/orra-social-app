...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Nav - 5 items + Menu — hidden when Go Live or streaming */}
      <nav className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 glass-panel border-t border-white/10 safe-area-bottom transition-transform duration-300 ease-in-out ${(navVisible && !isLiveActive && !showGoLiveSetup) ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="flex items-center justify-around px-1 py-1">
          ...
          <button
            ...
            className={`relative flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all ${
              currentView === 'live' ? 'text-red-400' : 'text-slate-500'
            }`}
          >
