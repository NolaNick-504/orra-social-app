...
import { useReels, useCreateReel, useToggleLike, useToggleSave } from '@/lib/api-hooks';
import { resolveImageUrl } from '@/lib/utils';
import { Heart, MessageCircle, Share2, Play, Radio, Music, Bookmark, UserPlus, BadgeCheck, Zap, Plus, X, Upload, Loader2, Video, Eye, StopCircle, MonitorUp } from 'lucide-react';
...
const categories = ['For You', 'Trending', 'Music', 'Dance', 'Comedy', 'Sports', 'Art', 'Lifestyle'];

const reelCategories = ['Trending', 'Dance', 'Music', 'Comedy', 'Art', 'Sports', 'Lifestyle'];

export function PrismReels() {
  const [activeCategory, setActiveCategory] = useState('For You');
  const { likedReels, toggleReelLike, toggleReelViewer, searchQuery, toggleFollow, followedUsers, savedReels, toggleSaveReel, auraTokens, markReelViewed,
    isLiveActive, currentLiveReelId, liveViewerCount, startLive, endLive, setLiveViewerCount } = useAuraStore();
  const queryClient = useQueryClient();

  // API hooks for persisting likes/saves to database
  ...
  const [liveStreamRef, setLiveStreamRef] = useState<MediaStream | null>(null);
  const liveVideoRef = useRef<HTMLVideoElement>(null);

  // Simulated viewer count increment
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (increment > 0) {
        setLiveViewerCount(current + increment);
      }
    }, 3000);
  ...
  const allReels = (() => {
    if (!reelsData) return [];
    const raw = Array.isArray(reelsData) ? reelsData : reelsData?.reels || [];
  ...
  const handleLike = (reelId: string) => {
    ...
    // Persist to database
    toggleLikeApi.mutate({ targetId: reelId, targetType: 'reel' });
    if (!isLiked) {
      if (!alreadyEarned) {
  ...
  const handleSave = (reelId: string) => {
    ...
    // Persist to database
    toggleSaveApi.mutate({ targetId: reelId, targetType: 'reel' });
  ...
  const handleFollow = async (userId: string) => {
    ...
    try {
      ...
      if (res.status === 403) {
        toggleFollow(userId);
        toast.error("Can't unfollow the ORRA Founder", { duration: 1500 });
        return;
      }
  ...
  const handleCameraVideoCaptured = (blob: Blob) => {
    ...
    const file = new File([blob], `camera-reel.${extension}`, { type: blob.type });
    setReelVideoFile(file);
    setReelVideoPreview(URL.createObjectURL(blob));
    setShowCamera(false);
  };
  ...
  const startCameraPreview = useCallback(async () => {
    try {
      ...
      setLiveStreamRef(stream);
      if (liveVideoRef.current) {
        liveVideoRef.current.srcObject = stream;
      }
    } catch (err) {
      console.error('Camera access failed:', err);
      toast.error('Camera access denied. You can still go live without a camera preview.');
    }
  }, []);

  const stopCameraPreview = useCallback(() => {
    if (liveStreamRef) {
      liveStreamRef.getTracks().forEach((track) => track.stop());
      setLiveStreamRef(null);
    }
  }, [liveStreamRef]);
  ...
  const handleStartLive = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/reels-live', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: liveTitle.trim(), category: liveCategory }),
      });
  ...
  const [showLiveSaveDialog, setShowLiveSaveDialog] = useState(false);
  const [isProcessingLiveAction, setIsProcessingLiveAction] = useState(false);

  const handleEndLiveAction = async (action: 'post' | 'save' | 'discard') => {
    setIsProcessingLiveAction(true);

    // Always stop camera and end live locally first
    stopCameraPreview();
    endLive();

    try {
      // If we have the reelId in state, use it directly
      ...
      if (reelIdToEnd) {
        await fetch('/api/reels-live', {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ reelId: reelIdToEnd, action }),
        });
      }

      setShowLiveSaveDialog(false);
      queryClient.invalidateQueries({ queryKey: ['reels'] });
      ...
      if (action === 'post') {
        ...
        toast.success('Stream discarded.');
      }
    } catch (error) {
  ...
  const handleEndLive = async () => {
    // Show save dialog instead of ending immediately
    setShowLiveSaveDialog(true);
  };

  // Start camera preview when Go Live modal opens
  useEffect(() => {
    if (showGoLive) {
      startCameraPreview();
    } else {
  ...
  }, [showGoLive, startCameraPreview, stopCameraPreview]);
  ...
  return (
    <div className="fade-in space-y-4 pb-4">
      {/* Token Balance + Create Reel + Go Live */}
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gradient-to-r from-red-600 to-red-500 text-white text-xs font-bold hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed relative"
          >
      ...
      {/* Currently Live Stream Card - shown when user is live */}
      {isLiveActive && !showLiveSaveDialog && (
          ...
          <div className="relative p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-1 rounded-lg bg-red-600/80 text-xs font-bold text-white animate-pulse flex items-center gap-1">
                  <Radio className="w-3 h-3" /> LIVE
                </span>
                <span className="text-sm font-bold text-white">Your Live Stream</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white/10 text-xs text-white">
            ...
            {/* Camera preview */}
            <div className="relative rounded-xl overflow-hidden aspect-video bg-black/40 mb-4">
              <video
                ref={liveVideoRef}
                autoPlay
                playsInline
              ...
              <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2 py-1 rounded-md bg-red-600/90 backdrop-blur-sm">
              ...
              <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 rounded-md bg-black/60 backdrop-blur-sm">
                ...
                <span className="text-[10px] font-bold text-white">{liveViewerCount.toLocaleString()}</span>
              </div>
            </div>

            <button
              onClick={handleEndLive}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-red-600/80 hover:bg-red-600 text-white font-bold text-sm transition-all"
            >
      ...
      {/* Save Dialog for Live Stream */}
      {isLiveActive && showLiveSaveDialog && (
        <div className="glass-panel rounded-2xl p-5 border border-white/10 fade-in">
          <div className="text-center mb-5">
            <div className="w-12 h-12 rounded-full bg-red-600/20 flex items-center justify-center mx-auto mb-3">
              <MonitorUp className="w-6 h-6 text-red-400" />
            </div>
            <h3 className="text-white font-bold text-base mb-1">End Live Stream?</h3>
            <p className="text-slate-400 text-xs">What would you like to do with your stream?</p>
          </div>
          <div className="space-y-2">
            <button
              onClick={() => handleEndLiveAction('post')}
              disabled={isProcessingLiveAction}
              className="w-full py-2.5 px-4 rounded-xl bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white font-semibold text-sm transition-all active:scale-[0.98]"
            >
              {isProcessingLiveAction ? 'Processing...' : 'Post to Feed'}
            </button>
            <button
              onClick={() => handleEndLiveAction('save')}
              disabled={isProcessingLiveAction}
              className="w-full py-2.5 px-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 disabled:opacity-50 text-white font-semibold text-sm transition-all active:scale-[0.98]"
            >
              {isProcessingLiveAction ? 'Processing...' : 'Save to Profile'}
            </button>
            <button
              onClick={() => handleEndLiveAction('discard')}
              disabled={isProcessingLiveAction}
              className="w-full py-2.5 px-4 rounded-xl hover:bg-red-500/10 disabled:opacity-50 text-red-400 font-semibold text-sm transition-all active:scale-[0.98]"
            >
              {isProcessingLiveAction ? 'Processing...' : 'Discard'}
            </button>
            <button
              onClick={() => setShowLiveSaveDialog(false)}
              disabled={isProcessingLiveAction}
              className="w-full py-1.5 text-slate-500 text-xs hover:text-slate-400 transition-all"
            >
              Cancel
            </button>
          </div>
      ...
      {/* Go Live Modal */}
      {showGoLive && (
        <div className="glass-panel rounded-2xl p-5 border border-red-500/20 fade-in space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Go Live</h3>
            </div>
            <button
              onClick={() => { setShowGoLive(false); setLiveTitle(''); setLiveCategory('Trending'); }}
          ...
          <div className="relative rounded-xl overflow-hidden aspect-video bg-black/40">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              {!liveVideoRef.current?.srcObject && (
                <div className="flex flex-col items-center gap-2 text-white/40">
                  <Video className="w-10 h-10" />
                  <span className="text-xs">Camera preview loading...</span>
                </div>
              )}
            </div>
          </div>
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-red-500/50 transition-all"
          />
          ...
          <button
            ...
            disabled={isStartingLive || !liveTitle.trim()}
            className="w-full px-6 py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-red-500 text-white font-bold hover:opacity-90 transition-all text-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isStartingLive ? (
              <>
          ...
          {reelVideoPreview ? (
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-white/5 border border-dashed border-white/20 text-sm text-slate-400 hover:text-white hover:border-violet-500/50 transition-all"
              >
      ...
      {filteredReels.length > 0 && (
        <div
          ...
          onClick={() => {
            toggleReelViewer(filteredReels[0].id);
            // Track featured reel view
            if (!useAuraStore.getState().viewedReels.has(filteredReels[0].id)) {
              markReelViewed(filteredReels[0].id);
              fetch(`/api/reels/${filteredReels[0].id}`).catch(() => {});
            }
          }}
        >
        ...
        {filteredReels.slice(1).map((reel: any) => {
          ...
          return (
            <div
              ...
              onClick={() => {
                toggleReelViewer(reel.id);
                // Track reel view for token reward + increment view count server-side
                if (!useAuraStore.getState().viewedReels.has(reel.id)) {
                  markReelViewed(reel.id);
                  // Increment view count on the server (non-blocking, best-effort)
