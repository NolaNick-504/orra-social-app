...
export function useInfinitePosts(vibeTag?: string) {
  return useInfiniteQuery<{
    posts: Post[];
    pagination: { page: number; limit: number; total: number; pages: number };
  }>({
    queryKey: ['posts', 'infinite', vibeTag],
    queryFn: ({ pageParam }) => {
      const params = new URLSearchParams();
      if (vibeTag) params.set('vibeTag', vibeTag);
...
export function useUserPosts(userId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiFetch<{ posts: Post[]; pagination: { page: number; limit: number; total: number; pages: number } }>(`/api/posts?authorId=${userId}&limit=50`),
    enabled: !!userId,
    staleTime: 1000 * 15,
...
export function useCreateReel() {
  ...
  return useMutation({
    mutationFn: (data: { title: string; thumbnail?: string; videoUrl: string; category?: string; song?: string; videoFile?: { data: string; filename: string; contentType: string } }) =>
...
export function useToggleLike() {
  ...
  return useMutation({
    mutationFn: ({ targetId, targetType, reactionType }: { targetId: string; targetType: string; reactionType?: string }) =>
      apiFetch('/api/likes', {
        method: 'POST',
        body: JSON.stringify({ targetId, targetType, reactionType: reactionType || 'like' }),
      }),
    onSuccess: (_data: any, variables: { targetId: string; targetType: string; reactionType?: string }) => {
      queryClient.invalidateQueries({ queryKey: ['posts'] });
      queryClient.invalidateQueries({ queryKey: ['reels'] });
...
export function useComments(postId: string | null, repostId?: string | null) {
  const repostParam = repostId ? `&repostId=${repostId}` : '';
  return useQuery({
    queryKey: ['comments', postId, repostId || 'original'],
    queryFn: () => apiFetch<{ comments: { id: string; text: string; author: PostAuthor; createdAt: string }[] }>(`/api/comments?postId=${postId}&limit=50${repostParam}`),
    enabled: !!postId,
    staleTime: 1000 * 10,
    refetchInterval: 1000 * 10, // Auto-refetch every 10s to pick up auto-responses
  });
}
...
export function useCreateComment() {
  ...
  return useMutation({
    mutationFn: ({ postId, text, repostId, parentId, replyToName }: { postId: string; text: string; repostId?: string | null; parentId?: string | null; replyToName?: string | null }) =>
      apiFetch('/api/comments', {
        method: 'POST',
        body: JSON.stringify({ postId, text, repostId: repostId || null, parentId: parentId || null, replyToName: replyToName || null }),
      }),
    onSuccess: (_, variables) => {
      // Invalidate both comments and posts so the new comment and count show up
      queryClient.invalidateQueries({ queryKey: ['comments', variables.postId, variables.repostId || 'original'] });
      queryClient.invalidateQueries({ queryKey: ['posts'] });
    },
  });
}
...
export function useReels(category?: string) {
  ...
  return useQuery({
    ...
    queryFn: () => apiFetch<{ reels: ReelData[]; pagination: { page: number; limit: number; total: number; pages: number } }>(`/api/reels?${params.toString()}`),
    staleTime: 1000 * 60,
  });
}
...
export function useReelHistory() {
  return useQuery({
    queryKey: ['reelHistory'],
    queryFn: () => apiFetch<ReelHistoryEntry[]>('/api/reel-history'),
    staleTime: 1000 * 30,
  });
}
...
export function useRecordReelView() {
  ...
  return useMutation({
    mutationFn: (data: { reelId: string; watchProgress?: number }) =>
      apiFetch('/api/reel-history', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reelHistory'] });
    },
  });
}
